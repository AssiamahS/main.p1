...
def scrape_videos(url):
    try:
        ...
        if response.status_code == 200:
            ...
            iframe_elements = soup.find_all('iframe')  # Adjust this based on the actual HTML structure

            # Extract video links from the 'src' attribute of the iframes
            video_links = [iframe['src'] for iframe in iframe_elements if 'src' in iframe.attrs]
